...
const ExperienceCard = ({ jobTitle,companyImage, companyName, techImages, jobDates, summaryPoints }: Props) => {
  return (
    <article className="flex flex-col rounded-sm items-center flex-shrink-0 w-48 md:w-64 xl:w-80 bg-gray-300 hover:bg-gray-400 dark:bg-gray-700 dark:hover:bg-gray-800 dark:text-gray-400 cursor-pointer transition-all  sm:w-full">
    <motion.img
  ...
  <div className="px-8 md:px-8 text-justify">
      ...
      <div className="overflow-x-hidden overflow-y-hidden h-fit">
        <ul className=" list-disc space-y-4 ">
          {summaryPoints.map((point, index) => (
            <li key={index} className="text-xs md:text-sm pb-2 text-justify">{point}</li>
          ))}
        </ul>
      </div>
    </div>
